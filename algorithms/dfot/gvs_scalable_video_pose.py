...
class GVSScalableVideoPose(DFoTVideoPose):
    """
    A child class of DFoTVideoPose that implements a scalable version of Generative View Stitching (GVS), which denoises context windows one-by-one, as opposed to all of them in parallel.
    """
    ...
    def _predict_videos(
        ...
        # 2) create chunk triplets: (past_chunk, chunk_to_denoise / target_chunk, future_chunk)
        ...
        # create chunk triplets using indices: this will result in an array of shape (B, num_windows*self.max_tokens, ...)
        # xs_pred.shape = (B, T, self.x_shape) -> xs_pred_chunk_triplets.shape = (B, num_windows*self.max_tokens, self.x_shape)
        # conditions.shape = (B, T, ...) -> conditions_chunk_triplets.shape = (B, num_windows*self.max_tokens, ...)
        # scheduling_matrix.shape = (num_denoising_steps, B, T) -> from_noise_levels_chunk_triplets.shape = (num_denoising_steps, B, num_windows*self.max_tokens)
        num_denoising_steps = scheduling_matrix.shape[0]
        ...
        pbar = tqdm(
            ...
            initial=0,
            desc="Scalable Generative View Stitching (GVS)",
            leave=False,
        )

        # iterate over denoising steps
        for m in range(num_denoising_steps - 1):
            indices_all_windows = indices_to_cycle_through_per_denoising_step[:, m]
            from_noise_levels_chunk_triplets_all_windows = (
                scheduling_matrix_chunk_triplets[m]
            )
            to_noise_levels_chunk_triplets_all_windows = (
                scheduling_matrix_chunk_triplets[m + 1]
            )

            indices_all_windows = rearrange(
                indices_all_windows,
                "b (windows max_tokens) -> b windows max_tokens",
                windows=num_windows,
                max_tokens=self.max_tokens,
            )
            from_noise_levels_chunk_triplets_all_windows = rearrange(
                from_noise_levels_chunk_triplets_all_windows,
                "b (windows max_tokens) -> b windows max_tokens",
                windows=num_windows,
                max_tokens=self.max_tokens,
            )
            to_noise_levels_chunk_triplets_all_windows = rearrange(
                to_noise_levels_chunk_triplets_all_windows,
                "b (windows max_tokens) -> b windows max_tokens",
                windows=num_windows,
                max_tokens=self.max_tokens,
            ...
            # noise tensor for current denoising step m
            ...
            for w in range(num_windows):
                indices = indices_all_windows[:, w]  # shape = (B, self.max_tokens)

                assert (
                    self.cfg.tasks.prediction.history_guidance.name == "vanilla"
                ), "only vanilla history guidance is supported for scalable GVS"
                # generate context_mask_chunk_triplets.shape = (B, self.max_tokens)
                ...
                if w == 0:
                    context_mask_chunk_triplets[
                        ..., chunk_sizes[0] : chunk_sizes[0] + self.n_context_tokens
                    ] = 1  # context frames

                # 5) coordinate the noise levels for all chunks
                from_noise_levels_chunk_triplets = (
                    from_noise_levels_chunk_triplets_all_windows[:, w]
                )
                to_noise_levels_chunk_triplets = (
                    to_noise_levels_chunk_triplets_all_windows[:, w]
                )

                # extract xs_pred_chunk_triplets from xs_pred
                # shape = (B, self.max_tokens, self.x_shape)
                ...
                with history_guidance(
                    ...
                    assert (
                        nfe == 2 or nfe == 1
                    ), "assume that history_guidance_manager is a SimpleHistoryGuidanceManager or there is no required guidance"
                    ...
                    noise_chunk_triplets = noise[
                        torch.arange(noise.shape[0], device=self.device)[:, None],
                        indices,
                    ]
                    ...
                    # removed conditions manipulation for penrose staircase
                    ...
                    ), "reconstruction guidance is not supported for scalable GVS"

                    # indices.shape = (B, self.max_tokens)
                    ...
                    # xs_pred_chunk_triplets, x0s_pred_chunk_triplets = self.diffusion_model.sample_step_windows_history_guidance(
                    ...
                    #
                    # xs_pred_chunk_triplets.shape = (B, (T//3 - 2)*self.max_tokens, self.x_shape)
                    ...
                    xs_pred_placeholder = torch.vmap(index_reduce_mean)(
                        xs_pred_placeholder,
                        indices_to_denoise,
                        xs_pred_chunk_to_denoise,
                    )  # shape = (B, T, self.x_shape)
            ...
            xs_pred = torch.where(
                self._extend_x_dim(context_mask) == 0, xs_pred_placeholder, xs_pred_prev
            )
            pbar.update(1)
