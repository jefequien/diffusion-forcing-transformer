...
class GVSVideoPose(DFoTVideoPose):
    """
    A child class of DFoTVideoPose that implements Generative View Stitching (GVS), a non-autoregressive sampling algorithm for camera-guided video generation.
    """
    ...
    def _predict_videos(
        ...
        pbar = tqdm(
            ...
            initial=0,
            desc="Generative View Stitching (GVS)",
            leave=False,
        )
        ...
        for m in range(num_denoising_steps - 1):
            ...
            ), "only vanilla history guidance is supported for GVS"
            ...
            to_noise_levels_chunk_triplets_input = scheduling_matrix_chunk_triplets[m + 1]

            # create a backup with all context tokens unmodified
            xs_pred_prev = xs_pred.clone()

            # Slice
            use_slice = m < self.cfg.diffusion.switch_timestep
            if use_slice:
                # Create slice backup
                ...
                slice_indices = torch.tensor([0,0,1,1,1,1,0,0] * num_windows, device=self.device, dtype=torch.bool)
                xs_pred = xs_pred[:, 2:-2]
                ...
                num_windows = math.ceil((target_length - 1) / 7)
            ...
            with history_guidance(
                ...
                (
                    ...
                    xs_pred_chunk_triplets,
                    from_noise_levels_chunk_triplets_input,
                    to_noise_levels_chunk_triplets_input,
                ...
                ), "reconstruction guidance is not supported for parallel sampling with history guidance"
                ...
                if self.cfg.viz_denoising:
                    record_xs_pred.append(xs_pred_chunk_triplets.clone().detach())
                    record_x0s_pred.append(x0s_pred_chunk_triplets.clone().detach())
        ...
        if self.cfg.viz_denoising:
            record_xs_pred = torch.stack(
                record_xs_pred, dim=0
            )  # shape = (num_denoising_steps, B, num_windows*max_tokens, self.x_shape)
            record_x0s_pred = torch.stack(
                record_x0s_pred, dim=0
            )  # shape = (num_denoising_steps, B, num_windows*max_tokens, self.x_shape)

            # unnormalize the denoising trajectories
            record_xs_pred = self._unnormalize_x(record_xs_pred)
            record_x0s_pred = self._unnormalize_x(record_x0s_pred)

            # before visualizing the denoising trajectories
            # each video has to be of shape (b, diffusion_steps, C, num_windows*H, max_tokens*W)
            record_xs_pred = rearrange(
                record_xs_pred,
                "m b (windows t) c h w -> b m c (windows h) (t w)",
                windows=num_windows,
                t=self.max_tokens,
            )
            record_x0s_pred = rearrange(
                record_x0s_pred,
                "m b (windows t) c h w -> b m c (windows h) (t w)",
                windows=num_windows,
                t=self.max_tokens,
            ...
            # plot xs_pred and x0s_pred
            log_video(
                record_xs_pred,
                step=self.global_step if hasattr(self, "global_step") else 0,
                namespace="denoising_traj_latent",
                logger=self.logger.experiment,
                indent=self.num_logged_videos,
                captions=f"each row = sliding window \n each column = time step",
                fps=10,
            )
            log_video(
                record_x0s_pred,
                step=self.global_step if hasattr(self, "global_step") else 0,
                namespace="denoising_traj_x0_pred",
                logger=self.logger.experiment,
                indent=self.num_logged_videos,
                captions=f"each row = sliding window \n each column = time step",
                fps=10,
